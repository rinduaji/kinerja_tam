	...
	?>
 	...
 	<section id="main-content">
 		...
 		<section class="wrapper">
 			...
 			<form id="form" name="demoform" method="post" action="form_ideas_wo_dev1.php">
 														...
 														<tbody>
 															<?php
																...
																// while ($qq <= date('m')) {
																	...
																	(SELECT t.UPD_AGENT,t.NAMA_MASTER,t.ALAMAT_MASTER,t.NO_KONTAK,t.TGL_CALL,t.HUB_PENERIMA,t.PENERIMA,t.TRANS_ID,td.KENDALA,t.GIMMICK,tnd.FCR,td.TODO,t.KETERANGAN,td.ND,td.TGL_JANJI_BAYAR FROM TRANS AS t INNER JOIN TRANS_DETAIL AS td ON t.TRANS_ID = td.TRANS_ID LEFT JOIN TICKET_NOSSA_DEV as tnd ON t.TRANS_ID = tnd.TRANS_ID 
																	...
																	FROM TRANS WHERE STATUS_CALL='CONTACTED' AND REASON_CALL ='SETUJU' AND LUP between '$range_tanggal1' 
																	and '$range_tanggal2' 
																	...
																	ORDER BY UPD_AGENT,NO_KONTAK ASC) AS f JOIN TRANS_DETAIL AS ttd ON f.TRANS_ID= ttd.TRANS_ID LEFT JOIN TICKET_NOSSA_DEV as tnd ON ttd.TRANS_ID = tnd.TRANS_ID  GROUP BY f.TRANS_ID ORDER BY f.UPD_AGENT ASC)");
																...
																if ($date_sekarang == date("D", strtotime("Mon"))) {
 																...
 																<?php
																	...
																	if ($total_tanggal == 0) {
																		...
																		$sql_qco = mysqli_query($conn, "SELECT qco,agent FROM app_kinerja_ideas WHERE tanggal= '$date_minggu'");
 																...
 																<?php
																	...
																	// while ($qq <= date('m')) {
																		...
																		ORDER BY UPD_AGENT,NO_KONTAK ASC) AS f JOIN TRANS_DETAIL AS ttd ON f.TRANS_ID= ttd.TRANS_ID LEFT JOIN TICKET_NOSSA_DEV as tnd ON ttd.TRANS_ID = tnd.TRANS_ID  GROUP BY f.TRANS_ID ORDER BY f.UPD_AGENT ASC)";
																		// $sql=mysqli_query($conn,"Select area,tanggal, COUNT(*) FROM app_kinerja_nilai  WHERE tanggal LIKE '%$qq%' GROUP BY area");
																		// print_r($query_sql_three);
																		$sql = mysqli_query($conn, $query_sql_three);
																	...
																	if ($total_tanggal == 0) {
																		...
																		$sql_qco = mysqli_query($conn, "SELECT qco,agent FROM app_kinerja_ideas WHERE tanggal= '$date_sabtu'");
