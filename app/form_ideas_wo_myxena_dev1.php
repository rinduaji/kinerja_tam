	...
	?>
 	...
 	<section id="main-content">
 		...
 		<section class="wrapper">
 			...
 			<form id="form" name="demoform" method="post" action="form_ideas_wo_myxena_dev1.php">
 									...
 									<div class="col-lg-12">
 										<section class="panel">
 											<div class="revenue-head" style="background: #695a70;-webkit-border-radius: 4px 4px 0px 0px;border-radius: 4px 4px 0px 0px;  color: #fff;line-height: 50px;">
 												<h3><i>Data MyXena Monitoring</i></h3>
 											</div>
 													...
 													<table data-toggle="table" class="table table-striped table-bordered table-hover header-fixed" id="sample_xx">
 														...
 														<thead>
 															<tr>
																...
																<th style="padding-top: 3px; padding-bottom: 3px; background-color: #EAEAEA !important">
																	<font face="Verdana" style="font-size: 9pt">GIMMICK
																	</font>
																</th>
																<th style="padding-top: 3px; padding-bottom: 3px; background-color: #EAEAEA !important">
																	<font face="Verdana" style="font-size: 9pt">FCR
																	</font>
																</th>
																<th style="padding-top: 3px; padding-bottom: 3px; background-color: #EAEAEA !important">
																	<font face="Verdana" style="font-size: 9pt">TODO
																	</font>
																</th>
																<th style="padding-top: 3px; padding-bottom: 3px; background-color: #EAEAEA !important">
																	<font face="Verdana" style="font-size: 9pt">CATATAN
																	</font>
																</th>
																<th style="padding-top: 3px; padding-bottom: 3px; background-color: #EAEAEA !important">
 														...
 														<tbody>
 															<?php
																...
																$data_jadi = array();
																// $sql_qco = mysqli_query($conn, "SELECT qco,agent FROM app_kinerja_myxena WHERE tanggal= '$date_now'");
																...
																// while ($qq <= date('m')) {
                                                                    ...
                                                                    $query_xena = "
																	(SELECT t.UPD_AGENT,t.NAMA_MASTER,t.ALAMAT_MASTER,t.NO_KONTAK,t.TGL_CALL,t.HUB_PENERIMA,t.PENERIMA,t.TRANS_ID,td.KENDALA,t.GIMMICK,tnd.FCR,td.TODO,t.KETERANGAN,td.ND,td.TGL_JANJI_BAYAR FROM trans_history AS t INNER JOIN trans_detail_history AS td ON t.TRANS_ID = td.TRANS_ID LEFT JOIN TICKET_NOSSA_DEV as tnd ON t.TRANS_ID = tnd.TRANS_ID 
																	WHERE t.STATUS_CALL='CONTACTED' AND t.REASON_CALL ='SETUJU' AND t.LUP between '$range_tanggal1' and '$range_tanggal2'
																	GROUP BY t.UPD_AGENT HAVING COUNT(*) < 2 ORDER BY COUNT(*) ASC)
																	UNION 
									
																	(SELECT f.UPD_AGENT,f.NAMA_MASTER,f.ALAMAT_MASTER,f.NO_KONTAK,f.TGL_CALL,f.HUB_PENERIMA,f.PENERIMA,f.TRANS_ID,ttd.KENDALA,f.GIMMICK,tnd.FCR,ttd.TODO,f.KETERANGAN,ttd.ND,ttd.TGL_JANJI_BAYAR
																	FROM 
																	(SELECT d.UPD_AGENT,d.NAMA_MASTER,d.ALAMAT_MASTER,d.NO_KONTAK,d.TGL_CALL,d.HUB_PENERIMA,d.PENERIMA,d.TRANS_ID,d.GIMMICK,d.KETERANGAN
																	FROM trans_history AS d  
																	JOIN (
																	SELECT max(d.NO_KONTAK) AS m, d.UPD_AGENT
																	FROM trans_history as d
																	JOIN (
																	SELECT max(NO_KONTAK) AS m, UPD_AGENT
																	FROM trans_history WHERE STATUS_CALL='CONTACTED' AND REASON_CALL ='SETUJU' AND LUP between '$range_tanggal1' 
																	and '$range_tanggal2' 
																	 
																	GROUP BY UPD_AGENT
																	) AS e ON d.UPD_AGENT=e.UPD_AGENT AND e.m>d.NO_KONTAK WHERE STATUS_CALL='CONTACTED' AND REASON_CALL ='SETUJU' 
																	AND LUP between '$range_tanggal1' and '$range_tanggal2' 
																	 
																	GROUP BY d.UPD_AGENT ASC
																	) AS e ON d.NO_KONTAK >= e.m AND d.UPD_AGENT = e.UPD_AGENT 
																	WHERE STATUS_CALL='CONTACTED' AND REASON_CALL ='SETUJU' AND LUP between '$range_tanggal1' and '$range_tanggal2' 
																	ORDER BY UPD_AGENT,NO_KONTAK ASC) AS f JOIN trans_detail_history AS ttd ON f.TRANS_ID= ttd.TRANS_ID LEFT JOIN TICKET_NOSSA_DEV as tnd ON ttd.TRANS_ID = tnd.TRANS_ID  GROUP BY f.TRANS_ID ORDER BY f.UPD_AGENT ASC)";
																	$sql = mysqli_query($conn_xena, $query_xena);

																	//echo "Select  area,tanggal, COUNT(*) FROM app_kinerja_nilai  WHERE tanggal LIKE '%$qq%' GROUP BY area";
																	while ($row = mysqli_fetch_row($sql)) {
																		# code...																	
																		$data[$i] = array($row[0], $row[1], $row[2], $row[3], $row[4], $row[5], $row[6], $row[7], $row[8], $row[9], $row[10], $row[11], $row[12], $row[13], $row[14]);
																		// $data_agent[$i] = array($row[0]);
																		array_push($data_agent, $row[0]);
																...
																$pkatt = mysqli_query($conn, "SELECT tanggal FROM app_kinerja_myxena WHERE tanggal = '$date_now'");
																...
																if ($total_tanggal == 0) {
																	...
																	for ($i = 0; $i < $total1; $i++) {
																		for ($j = 0; $j < $total; $j++) {
																			...
																			$sql_insert_agent = "INSERT INTO app_kinerja_myxena (qco,agent,area,tanggal) VALUES ('$nama_qco[$i]','$parameter1','$parameter2','$parameter3')";
																	...
																	if ($mod != '') {
																		...
																		for ($i = $k; $i < $total2; $i++) {
																			...
																			$sql_insert_agent_mod = "INSERT INTO app_kinerja_myxena (qco,agent,area,tanggal) VALUES ('$nqco','$parameter1','$parameter2','$parameter3')";
																	...
																	$sql_qco = mysqli_query($conn, "SELECT qco,agent FROM app_kinerja_myxena WHERE tanggal= '$date_now' AND qco= '$nama_login'");
																	// print_r("SELECT qco,agent FROM app_kinerja_myxena WHERE tanggal= '$date_now' AND qco= '$nama_login'");
																	//echo "Select  area,tanggal, COUNT(*) FROM app_kinerja_nilai  WHERE tanggal LIKE '%$qq%' GROUP BY area";
																	$angka_qco = 0;
																	$total_agent = mysqli_num_rows($sql_qco);
																	while ($row_qco = mysqli_fetch_row($sql_qco)) {
																...
																// if (count($data_kinerja_nilai) == 0) {
																	...
																	for ($i = 0; $i < $no; $i++) {
																		for ($j = 0; $j < $total_agent; $j++) {
																			...
																			if ($data_jadi[$j][1] == trim($data[$i][0]) and $_SESSION['name'] == $data_jadi[$j][0]) {

																				$data_fix[$i] = array(trim($data[$i][0]), $data[$i][1], $data[$i][2], $data[$i][3], $data[$i][4], $data[$i][5], $data[$i][6], $data[$i][7], $data[$i][8], $data[$i][9], $data[$i][10], $data[$i][11], $data[$i][12], $data[$i][13], $data[$i][14]);
																	...
																	for ($i = 0; $i < $no; $i++) {
																		...
																		for ($j = 0; $j < $angka1; $j++) {
                                                                            ...
                                                                            if ($data_kinerja_nilai[$j][0] != trim($data[$i][0]) and $data_kinerja_nilai[$j][1] != $data[$i][3]) {
                                                                                if ($j == $angka1 - 1 and $cek == true) {
                                                                                    // print_r($data_jadi[$k][1]);
                                                                                    // print_r($data[$i][0].' == '.$data_jadi[$k][1].' '.($data[$i][0] == $data_jadi[$k][1]).'<br>');
																...
																?>
 																	<tr class="odd gradeX">
 																		...
 																		<td style="padding-top: 2px; padding-bottom: 2px">
 																			<font face="Tahoma" style="font-size: 9pt"><?php echo $data_fix[$i][8]; ?>
 																			</font>
 																		</td>
																		 <td style="padding-top: 2px; padding-bottom: 2px">
																		 ...
																		 <td style="padding-top: 2px; padding-bottom: 2px">
 																			<font face="Tahoma" style="font-size: 9pt"><?php echo $data_fix[$i][10]; ?>
 																			</font>
 																		</td>
																		 <td style="padding-top: 2px; padding-bottom: 2px">
 																			<font face="Tahoma" style="font-size: 9pt"><?php echo $data_fix[$i][11]; ?>
 																			</font>
 																		</td>
																		<td style="padding-top: 2px; padding-bottom: 2px">
 																			<font face="Tahoma" style="font-size: 9pt"><?php echo $data_fix[$i][12]; ?>
 																			</font>
 																		</td>
																		<td style="padding-top: 2px; padding-bottom: 2px">
 																			<font face="Tahoma" style="font-size: 9pt"><?php echo $data_fix[$i][14]; ?>
 																			</font>
 																		</td>
 																		<td style="padding-top: 2px; padding-bottom: 2px">
 																			<a href="form_ideas_detail_dev1.php?agent=<?php echo $data_fix[$i][0]; ?>&telp=<?php echo $data_fix[$i][3]; ?>&gimmick=<?php echo $data_fix[$i][9]; ?>&fcr=<?php echo $data_fix[$i][10]; ?>&nd=<?php echo $data_fix[$i][13]?>" class="btn btn-primary btn-sm">Detail</a>
 																		</td>
																...
																// $sql_nilai_myxena_kosong = mysqli_query($conn, "SELECT tanggal FROM app_kinerja_myxena WHERE tanggal= '$tgl_sebelumnya'");
																// $total_nilai_myxena_kosong = mysqli_num_rows($sql_nilai_myxena_kosong);

																// $tgl_dua_sebelumnya = date("Y-m-d", strtotime('-2 day', strtotime($date_now)));
																// $sql_nilai_myxena_kosong_dua = mysqli_query($conn, "SELECT tanggal FROM app_kinerja_myxena WHERE tanggal= '$tgl_dua_sebelumnya'");
																// $total_nilai_myxena_kosong_dua = mysqli_num_rows($sql_nilai_myxena_kosong);
																...
																// if ($total_nilai_myxena_kosong == 0) {
																...
																// if ($total_nilai_myxena_kosong_dua == 0) {
																...
																if ($date_sekarang == date("D", strtotime("Mon"))) {
 																...
 																<?php
																	...
																	$pkatt = mysqli_query($conn, "SELECT tanggal FROM app_kinerja_myxena WHERE tanggal = '$date_minggu'");
																	$total_tanggal = mysqli_num_rows($pkatt);
																	if ($total_tanggal == 0) {
																		...
																		$sql_qco = mysqli_query($conn, "SELECT qco,agent FROM app_kinerja_myxena WHERE tanggal= '$date_minggu'");
 																...
 																<?php
																	...
																	if ($data_kinerja_nilai[0] == 0) {
																		...
																		for ($i = 0; $i < $no; $i++) {
																			...
																			for ($j = 0; $j < $angka1; $j++) {
																				...
																				if ($data_kinerja_nilai[$j][0] != $data[$i][0] and $data_kinerja_nilai[$j][1] != $data[$i][3]) {
																					if ($j == $angka1 - 1 and $cek == true) {

																						for ($k = 0; $k < $total_agent; $k++) {
																							if ($_SESSION['name'] == $data_jadi[$k][0]) {
																								if ($data_jadi[$k][1] == trim($data[$i][0])) {
																									$data_fix[$i] = array(trim($data[$i][0]), $data[$i][1], $data[$i][2], $data[$i][3], $data[$i][4], $data[$i][5], $data[$i][6], $data[$i][7], $data[$i][8], $data[$i][9], $data[$i][10], $data[$i][11], $data[$i][12], $data[$i][13], $data[$i][14]);
																								}
																							}
																						}
																	...
																	?>
																		 ...
																		 <td style="padding-top: 2px; padding-bottom: 2px">
 																			...
 																			<td style="padding-top: 2px; padding-bottom: 2px">
 																				<a href="form_ideas_detail_dev1.php?agent=<?php echo $data_fix[$i][0]; ?>&telp=<?php echo $data_fix[$i][3]; ?>&id=<?php echo $data_fix[$i][7]; ?>&gimmick=<?php echo $data_fix[$i][9]; ?>&fcr=<?php echo $data_fix[$i][10]; ?>&nd=<?php echo $data_fix[$i][13]; ?>" class="btn btn-primary btn-sm">Detail</a>
 																			</td>
 																...
 																<?php
																	...
																	$yesterday = date('Y-m-d', strtotime($date1 . "-3 days"));
																	$range_tanggal1 = $yesterday . " 00:00:00";
																	$range_tanggal2 = $yesterday . " 23:59:59";
																	...
																	// while ($qq <= date('m')) {
																		...
																		ORDER BY UPD_AGENT,NO_KONTAK ASC) AS f JOIN trans_detail_history AS ttd ON f.TRANS_ID= ttd.TRANS_ID LEFT JOIN TICKET_NOSSA_DEV as tnd ON ttd.TRANS_ID = tnd.TRANS_ID  GROUP BY f.TRANS_ID ORDER BY f.UPD_AGENT ASC)";
																		// $sql=mysqli_query($conn,"Select area,tanggal, COUNT(*) FROM app_kinerja_nilai  WHERE tanggal LIKE '%$qq%' GROUP BY area");
																		$sql = mysqli_query($conn_xena, $query_sql_three);
																		// echo $query_sql_three;
																		//echo "Select  area,tanggal, COUNT(*) FROM app_kinerja_nilai  WHERE tanggal LIKE '%$qq%' GROUP BY area";
																		while ($row = mysqli_fetch_row($sql)) {
																			# code...																	
																			$data[$i] = array($row[0], $row[1], $row[2], $row[3], $row[4], $row[5], $row[6], $row[7], $row[8], $row[9], $row[10], $row[11], $row[12], $row[13], $row[14]);
																	...
																	$pkatt = mysqli_query($conn, "SELECT tanggal FROM app_kinerja_myxena WHERE tanggal = '$date_sabtu'");
																	$total_tanggal = mysqli_num_rows($pkatt);
																	...
																	if ($total_tanggal == 0) {
																		...
																		$sql_qco = mysqli_query($conn, "SELECT qco,agent FROM app_kinerja_myxena WHERE tanggal= '$date_sabtu'");
